      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
                ...
                {education.coursework && (
                  <div className="pt-2.5 sm:pt-3 border-t border-zinc-800/80">
                    <div className="flex items-center justify-between mb-1.5 sm:mb-2">
                      <div className="flex items-center space-x-1.5 text-[10px] sm:text-[11px] font-mono uppercase tracking-wider text-zinc-400">
                    ...
                    >
                      {education.coursework.map((course) => (
                        <span
                          key={course}
                          className="px-2.5 py-0.5 rounded-lg bg-zinc-900/90 border border-zinc-800 text-[10px] sm:text-xs font-mono text-zinc-300 shrink-0 snap-start"
                        >
                          {course}
                        </span>
                      ))}
        ...
        {viewMode === 'timeline' && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -15 }}
            transition={{ duration: 0.35 }}
            className="relative border-l border-zinc-800/80 ml-4 sm:ml-8 pl-5 sm:pl-10 space-y-5 sm:space-y-8 max-h-[520px] sm:max-h-none overflow-y-auto pr-1 sm:pr-0 no-scrollbar"
          >
                ...
                <SpaceGlassPanel className="!p-4 sm:!p-6">
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 sm:gap-6">
                    <div className="space-y-2">
                      ...
                      <div className="flex sm:flex-wrap overflow-x-auto sm:overflow-visible gap-1.5 pt-1.5 no-scrollbar pb-0.5 snap-x">
                        {item.skills.map((skill) => (
                          <span key={skill} className="px-2 py-0.5 rounded-md bg-zinc-900 border border-zinc-800 text-[10px] font-mono text-zinc-300 shrink-0 snap-start">
                            {skill}
                          </span>
                        ))}
                      </div>
