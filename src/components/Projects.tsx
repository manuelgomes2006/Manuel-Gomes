  ...
  return (
    <section id="projects" className="py-24 bg-[#09090b] border-t border-zinc-900 relative scroll-mt-24">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        {/* 2 Main Project Showcase Cards with 3D Tilt */}
          ...
          {PERSONAL_DATA.projects.map((project, idx) => (
            <TiltCard key={project.id} maxTilt={5}>
              <motion.div
                ...
                className="rounded-3xl bg-gradient-to-br from-zinc-900/80 via-zinc-950/90 to-black/90 border border-zinc-800/80 p-6 sm:p-10 backdrop-blur-2xl hover:border-zinc-700/90 transition-all duration-500 shadow-2xl overflow-hidden group relative"
              >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center relative z-10">
                  ...
                  <div className="lg:col-span-7 space-y-5">
                    <div className="flex items-center space-x-3">
                      <div className="p-2.5 rounded-xl bg-zinc-800/90 border border-zinc-700/80 text-zinc-200 shadow-md">
                        {project.id === 'scholarmatch-ai' ? <Cpu className="w-5 h-5" /> : <Dumbbell className="w-5 h-5" />}
                      </div>
                      <span className="text-xs font-mono uppercase tracking-widest text-zinc-400 bg-zinc-950/90 px-3.5 py-1 rounded-full border border-zinc-800">
                    ...
                    <div className="flex flex-wrap gap-2 pt-1">
                      {project.tech.map((t) => (
                        <span
                          key={t}
                          className="px-3 py-1 rounded-lg bg-zinc-950/90 border border-zinc-800 text-xs font-mono text-zinc-300"
                        >
                          `{t}`
                        </span>
                      ))}
                    ...
                    <div className="relative rounded-2xl overflow-hidden bg-zinc-950 border border-zinc-800/90 p-2 shadow-inner group-hover:border-zinc-700 transition-all duration-500">
                      {project.image ? (
                        <img
                          ...
                          className="w-full h-56 sm:h-64 object-cover object-top rounded-xl filter contrast-105 group-hover:scale-105 transition-transform duration-700 ease-out transform-gpu"
                        />
                      ) : (
