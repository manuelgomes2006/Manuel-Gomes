...
import { Menu, X, Terminal, Code2, Cpu } from 'lucide-react';
    ...
    <header className="fixed top-0 left-0 right-0 z-40 p-4 sm:p-6 pointer-events-none font-mono">
      <div className="max-w-7xl mx-auto flex items-center justify-between pointer-events-auto">
        ...
        <a
          ...
          className="group flex items-center space-x-3 px-4 py-2 rounded-2xl bg-zinc-950/90 border border-zinc-800 backdrop-blur-xl shadow-2xl hover:border-emerald-500/60 transition-all duration-300 transform-gpu hover:scale-105"
        >
          <div className="w-8 h-8 rounded-xl bg-emerald-500 text-black flex items-center justify-center font-black text-xs shadow-md">
            &lt;/&gt;
          </div>
            ...
            return (
              <button
                ...
                className={`relative px-4 py-2 rounded-full text-[11px] font-mono tracking-widest uppercase transition-all duration-300 ${
                  isActive
                    ? 'text-emerald-400 font-bold bg-zinc-900 border border-emerald-500/50 shadow-md'
        ...
        <div className="flex items-center space-x-3">
          <button
            ...
            className="hidden sm:inline-flex items-center space-x-2 px-5 py-2.5 rounded-full bg-emerald-400 hover:bg-emerald-300 text-zinc-950 text-xs font-mono font-bold uppercase tracking-widest transition-all duration-300 shadow-xl transform-gpu hover:scale-105"
          >
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden fixed inset-x-4 top-20 bg-zinc-950/95 border border-zinc-800 rounded-3xl p-6 backdrop-blur-2xl shadow-2xl flex flex-col space-y-3 pointer-events-auto font-mono">
          <div className="flex items-center justify-between pb-3 border-b border-zinc-800 text-[10px] text-emerald-400 uppercase tracking-widest">
            ...
            <button
              ...
              className={`w-full py-3 px-4 rounded-xl text-xs font-mono tracking-widest uppercase text-left flex items-center justify-between transition-colors ${
                activeSection === link.id
                  ? 'bg-zinc-900 text-emerald-400 font-bold border border-emerald-500/50'
          ...
          <button
            onClick={() => scrollToSection('contact')}
            className="mt-3 w-full py-3.5 text-xs font-bold uppercase tracking-widest text-zinc-950 bg-emerald-400 rounded-xl shadow-lg flex items-center justify-center space-x-2 font-mono"
          >
